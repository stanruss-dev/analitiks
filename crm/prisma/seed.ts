...
async function main() {
  ...
  const pipeline = await prisma.pipeline.upsert({
    ...
    create: {
      ...
      stages: {
        create: [
          { name: 'Новая', order: 1, color: '#6366f1' },
          { name: 'Квалификация', order: 2, color: '#8b5cf6' },
          { name: 'Предложение', order: 3, color: '#f59e0b' },
          { name: 'Переговоры', order: 4, color: '#f97316' },
          { name: 'Закрыта', order: 5, color: '#10b981' },
        ],
      },
    },
